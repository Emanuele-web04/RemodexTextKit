...
public enum AttachmentSelectionStyle: Sendable {
  ...
  /// part of the selected range, RemodexTextKit dims the rendered attachment view.
  case object
}

/// Provides a view that can be embedded inline in attributed text.
///
/// Attachments are resolved from markup and rendered inline by RemodexTextKit. Use attachments to
...
///
/// RemodexTextKit uses `AnyAttachment` to store heterogeneous attachments in attributed content.
